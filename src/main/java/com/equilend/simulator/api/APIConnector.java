...
public class APIConnector {
	...
	private static Gson gson = new GsonBuilder()
			.registerTypeAdapter(OffsetDateTime.class,
					(JsonDeserializer<OffsetDateTime>) (json, typeOfT, context) -> OffsetDateTime
							.parse(json.getAsString()))
			.registerTypeAdapter(OffsetDateTime.class,
					(JsonSerializer<OffsetDateTime>) (offsetDateTime, type,
							jsonSerializationContext) -> new JsonPrimitive(offsetDateTime.format(formatter)))
	...
	private synchronized static HttpResponse<String> send(HttpRequest httpRequest) throws APIException {
		return send(httpRequest, 1);
	}

	private synchronized static HttpResponse<String> send(HttpRequest httpRequest, int retries) throws APIException {
		...
		try {
			...
			if (e.getMessage().contains("GOAWAY") && retries <= 3) {
				logger.info("Recreating httpClient after GOAWAY received. Retry " + retries);
				// retry the request with a new httpClient;
				httpClient = null;
				httpResponse = send(httpRequest, retries + 1);
			} else {
				logger.warn("HTTPClient unexpected exception. " + e.getMessage(), e);
				logger.warn(e.getMessage(), e);
				throw new APIException(e.getMessage(), e);
			}
		} catch (InterruptedException i) {
			logger.warn("HTTPClient send interrupted. " + i.getMessage(), i);
			throw new APIException(i.getMessage(), i);
		}

		return httpResponse;
	}

	private static boolean isSuccess(HttpResponse<String> response) throws APIException {
	...
	public static List<Event> getAllEvents(OneSourceToken token, OffsetDateTime since, Long eventId)
		...
		HttpResponse<String> getResponse = send(getRequest);
	...
	public static List<Loan> getAllLoans(OneSourceToken token, String status, String since) throws APIException {
		validateAPISetting(token);
	...
	public static int postLoanProposal(OneSourceToken token, LoanProposal loan) throws APIException {
		...
		try {
			postRequest = HttpRequest.newBuilder().uri(new URI(restAPIURL + "/loans"))
					...
					.header("Content-Type", "application/json").POST(BodyPublishers.ofString(loanJson)).build();
		} catch (URISyntaxException e) {
		...
		HttpResponse<String> postResponse = send(postRequest);
	...
	public static int acceptLoan(OneSourceToken token, String loanId, LoanProposalApproval loanProposalApproval)
			throws APIException {
		validateAPISetting(token);
		...
		try {
			postRequest = HttpRequest.newBuilder().uri(new URI(restAPIURL + "/loans/" + loanId + "/approve"))
					.header("Authorization", "Bearer " + token.getAccessToken())
					.header("Content-Type", "application/json").POST(BodyPublishers.ofString(settlementJson)).build();
		} catch (URISyntaxException e) {
	...
	public static int cancelPendingLoan(OneSourceToken token, String loanId) throws APIException {
		...
		HttpRequest postRequest;
		try {
			postRequest = HttpRequest.newBuilder().uri(new URI(restAPIURL + "/loans/" + loanId + "/cancelpending"))
					.header("Authorization", "Bearer " + token.getAccessToken())
					.POST(HttpRequest.BodyPublishers.noBody()).build();
		} catch (URISyntaxException e) {
	...
	public static int instructLoanSettlementStatus(OneSourceToken token, String loanId, SettlementStatus status)
			throws APIException {
		validateAPISetting(token);

		SettlementStatusUpdate settlementStatusUpdate = new SettlementStatusUpdate().settlementStatus(status);
		HttpResponse<String> patchResponse;
		try {
			String body = gson.toJson(settlementStatusUpdate);
			HttpRequest patchRequest = HttpRequest.newBuilder().uri(new URI(restAPIURL + "/loans/" + loanId))
					.header("Authorization", "Bearer " + token.getAccessToken())
	...
	public static int postRerateProposal(OneSourceToken token, String loanId, RerateProposal rerate)
		...
		try {
			postRequest = HttpRequest.newBuilder().uri(new URI(restAPIURL + "/loans/" + loanId + "/rerates"))
					.header("Authorization", "Bearer " + token.getAccessToken())
					.header("Content-Type", "application/json").POST(BodyPublishers.ofString(rerateJson)).build();
		} catch (URISyntaxException e) {
	...
	public static int cancelRerateProposal(OneSourceToken token, String loanId, String rerateId) throws APIException {
		validateAPISetting(token);
	...
	public static int cancelReratePending(OneSourceToken token, String loanId, String rerateId) throws APIException {
		validateAPISetting(token);
	...
	public static int approveRerateProposal(OneSourceToken token, String loanId, String rerateId) throws APIException {
		validateAPISetting(token);
	...
	public static int declineRerateProposal(OneSourceToken token, String loanId, String rerateId) throws APIException {
		validateAPISetting(token);
	...
	public static int proposeReturn(OneSourceToken token, String loanId, ReturnProposal returnProposal)
		...
		try {
			postRequest = HttpRequest.newBuilder().uri(new URI(restAPIURL + "/loans/" + loanId + "/returns"))
					.header("Authorization", "Bearer " + token.getAccessToken())
					.header("Content-Type", "application/json").POST(BodyPublishers.ofString(returnJson)).build();
		} catch (URISyntaxException e) {
	...
	public static BuyinComplete getBuyinById(OneSourceToken token, String buyinId) throws APIException {
		...
		HttpRequest getRequest;
		try {
			getRequest = HttpRequest.newBuilder().uri(new URI(restAPIURL + "/buyins/completes/" + buyinId))
					.header("Authorization", "Bearer " + token.getAccessToken()).build();
		} catch (URISyntaxException e) {
	...
	public static int acceptBuyin(OneSourceToken token, String loanId, String buyinId) throws APIException {
		...
		try {
			...
			String message = "Error with sending approve buyin post request for loan " + loanId + " buyin " + buyinId;
			logger.debug(message, e);
			throw new APIException(message, e);
		}
	...
	public static int proposeBuyin(OneSourceToken token, String loanId, BuyinCompleteRequest buyinCompleteRequest)
		...
		try {
			...
			postRequest = HttpRequest.newBuilder().uri(new URI(restAPIURL + "/loans/" + loanId + "/buyins/completes"))
					.header("Authorization", "Bearer " + token.getAccessToken())
					.header("Content-Type", "application/json").POST(HttpRequest.BodyPublishers.ofString(body)).build();
		} catch (URISyntaxException e) {
			String message = "Error with sending propose buyin post request for loan " + loanId;
			logger.debug(message, e);
			throw new APIException(message, e);
		}
	...
	public static Recall getRecallById(OneSourceToken token, String recallId) throws APIException {
		HttpRequest getRequest;
		try {
			getRequest = HttpRequest.newBuilder().uri(new URI(restAPIURL + "/recalls/" + recallId))
					.header("Authorization", "Bearer " + token.getAccessToken()).build();
		} catch (URISyntaxException e) {
	...
	public static int proposeRecall(OneSourceToken token, String loanId, RecallProposal recallProposal)
		...
		try {
			...
			postRequest = HttpRequest.newBuilder().uri(new URI(restAPIURL + "/loans/" + loanId + "/recalls"))
					.header("Authorization", "Bearer " + token.getAccessToken())
					.header("Content-Type", "application/json").POST(HttpRequest.BodyPublishers.ofString(body)).build();
		} catch (URISyntaxException e) {
	...
	public static LoanSplit getSplit(OneSourceToken token, String loanId, String splitId) throws APIException {
		HttpRequest getRequest;
		try {
			getRequest = HttpRequest.newBuilder().uri(new URI(restAPIURL + "/loans/" + loanId + "/split/" + splitId))
					.header("Authorization", "Bearer " + token.getAccessToken()).build();
		} catch (URISyntaxException e) {
	...
	public static int proposeSplit(OneSourceToken token, String loanId, List<LoanSplitLot> splitLots)
		...
		try {
			...
			postRequest = HttpRequest.newBuilder().uri(new URI(restAPIURL + "/loans/" + loanId + "/split"))
					.header("Authorization", "Bearer " + token.getAccessToken())
					.header("Content-Type", "application/json").POST(HttpRequest.BodyPublishers.ofString(body)).build();
		} catch (URISyntaxException e) {
	...
	public static int postRecallAck(OneSourceToken token, String loanId, String recallId,
			RecallAcknowledgement recallAcknowledgement) throws APIException {
		validateAPISetting(token);
		HttpRequest postRequest;
		try {
